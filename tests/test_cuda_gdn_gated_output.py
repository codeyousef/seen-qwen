...
class CudaGdnGatedOutputContractTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.test = (ROOT / "native/tests/gdn_gated_output_test.cu").read_text()
        cls.runner = (ROOT / "scripts/cuda/run_qwn_041d.sh").read_text()
    ...
    def test_surface_is_model_owned_and_fixed_width(self) -> None:
        symbol = "seen_qwen_gdn_gated_rms_norm_f32"
        for text in (self.header, self.source, self.wrapper):
            self.assertIn(symbol, text)
        for required in ("core", "gate", "weight", "rows", "width", "epsilon"):
            self.assertIn(required, self.header)

    def test_kernel_uses_exact_seen_stream_without_policy(self) -> None:
        self.assertIn("gdn_gated_rms_norm_f32<<<", self.source)
    ...
    def test_corpus_covers_official_geometry_differential_and_fail_closed_paths(self) -> None:
        for required in (
            "kOfficialHeads = 48", "kOfficialValueDim = 128",
            "Exact core/output alias", "Invalid geometry, bounds, device identity",
            "std::numeric_limits<uint64_t>::max()", "seen_cuda_graph_begin_capture",
            "SEEN_CUDA_INCOMPATIBLE",
        ):
            self.assertIn(required, self.test)
        self.assertIn("weight is applied directly", self.contract)
    ...
    def test_runner_is_hard_scoped_and_sanitized(self) -> None:
        for required in (
            ...
            "run_qwn_041c.sh", "qwn_041d_cuda_test", "compute-sanitizer",
