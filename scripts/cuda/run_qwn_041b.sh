...
ARTIFACT_ROOT="$ROOT_DIR/.seen/artifacts/qwn_041b"
...
if [ "${1:-}" != "--inner" ]; then
    exec env -u SEEN_DATA_PATH -u SEEN_STDLIB_PATH -u SEEN_RUNTIME_PATH \
        ...
        env QWN_041B_HARD_SCOPE=1 SEEN_TOOLCHAIN_ROOT="$TOOLCHAIN_ROOT" \
        "$0" --inner
fi

[ "${QWN_041B_HARD_SCOPE:-0}" = 1 ] || {
    echo "qwn-041b: verified hard scope is required" >&2
    exit 126
}
...
"$SEEN_BIN" check "$ROOT_DIR/tests/qwn_041b_gdn_decode_test.seen" --frozen
"$SEEN_BIN" compile "$ROOT_DIR/tests/qwn_041b_gdn_decode_test.seen" \
    "$ARTIFACT_ROOT/qwn_041b_seen_surface_test" \
...
for tool in memcheck initcheck racecheck synccheck; do
    ...
    /opt/cuda/bin/compute-sanitizer "${args[@]}" "$BUILD_ROOT/qwn_041b_cuda_test"
done

sha256sum "$BUILD_ROOT/libseen_qwen_cuda.so" "$BUILD_ROOT/qwn_041b_cuda_test" \
...
[ "$toolchain_hash_after" = "$toolchain_hash_before" ] || {
    echo "qwn-041b: compiler installation changed during verification" >&2; exit 126;
}
outside_objects_after=$(find "$ROOT_DIR" -path "$ROOT_DIR/.seen" -prune -o \
    -type f \( -name '*.o' -o -name '*.sig' -o -name '*.a' \) -print0 |
    sort -z | xargs -0 -r sha256sum | sha256sum | awk '{print $1}')
[ "$outside_objects_after" = "$outside_objects_before" ] || {
    echo "qwn-041b: compiler objects changed outside the ignored .seen root" >&2; exit 126;
}
echo "PASS: QWN-041B v0.20.4 recurrent GDN decode CUDA gates"
