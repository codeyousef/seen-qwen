...
ARTIFACT_ROOT="$ROOT_DIR/.seen/artifacts/qwn_044a"
...
[ "${QWN_044A_HARD_SCOPE:-0}" = 1 ] || exit 126
...
python3 -m unittest tests/test_cuda_projection_descriptor.py \
    tests/test_cuda_ffn_execution.py tests/test_cuda_lm_head_greedy.py
"$SEEN_PACKAGE_CLIENT" audit --lock Seen.lock
"$SEEN_BIN" check "$ROOT_DIR/tests/qwn_043a_projection_descriptor_test.seen" --frozen
"$SEEN_BIN" check "$ROOT_DIR/tests/qwn_044a_lm_head_test.seen" --frozen
"$SEEN_BIN" compile "$ROOT_DIR/tests/qwn_044a_lm_head_test.seen" \
    "$ARTIFACT_ROOT/qwn_044a_ir_only" --target-cpu=x86-64 \
    --no-cache --jobs 1 --opt-jobs 1 --no-fork --frozen \
    --emit-module-ir-dir "$IR_ROOT" --stop-after-ir
projection_ir=$(grep -l 'define .*QwenCudaLmHeadExecutor_executeGreedy' "$IR_ROOT"/*.ll)
[ -n "$projection_ir" ] || exit 1
execute_ir="$IR_ROOT/qwn_044a_execute.ll"
sed -n '/define .*QwenCudaLmHeadExecutor_executeGreedy/,/^}/p' "$projection_ir" > "$execute_ir"
[ "$(grep -c 'call %CudaNativeStatus @seen_cublaslt_matmul' "$execute_ir")" -eq 1 ]
[ "$(grep -c 'call %CudaNativeStatus @seen_qwen_greedy_argmax_low_precision' "$execute_ir")" -eq 1 ]
grep -q 'ptrtoint ptr .* ; &member' "$execute_ir"
...
mkdir -p "$SEEN_PROJECT/tests" "$SEEN_PROJECT/native/lib"
cp "$ROOT_DIR/tests/qwn_044a_lm_head_test.seen" \
    "$SEEN_PROJECT/tests/qwn_044a_lm_head_test.seen"
cp "$ROOT_DIR/tests/qwn_044a_project/Seen.toml" "$SEEN_PROJECT/Seen.toml"
...
"$SEEN_BIN" compile "$SEEN_PROJECT/tests/qwn_044a_lm_head_test.seen" \
    "$ARTIFACT_ROOT/qwn_044a_seen_test_fast" --target-cpu=x86-64 \
    --no-cache --jobs 1 --opt-jobs 1 --no-fork --offline
"$ARTIFACT_ROOT/qwn_044a_seen_test_fast"
"$SEEN_BIN" compile "$SEEN_PROJECT/tests/qwn_044a_lm_head_test.seen" \
    "$ARTIFACT_ROOT/qwn_044a_seen_test" --release --lto=thin \
    --target-cpu=x86-64 --no-cache --jobs 1 --opt-jobs 1 --no-fork --offline
"$ARTIFACT_ROOT/qwn_044a_seen_test"
...
for tool in memcheck initcheck racecheck synccheck; do
    ...
    /opt/cuda/bin/compute-sanitizer "${args[@]}" "$BUILD_ROOT/qwn_044a_cuda_test"
done
sha256sum "$BUILD_ROOT/qwn_044a_cuda_test" \
...
echo "PASS: QWN-044A v0.20.7 resident LM-head, deterministic greedy, RTX 4090, sanitizer, graph, and cleanup gates"
