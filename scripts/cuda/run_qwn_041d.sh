...
ARTIFACT_ROOT="$ROOT_DIR/.seen/artifacts/qwn_041d"
...
if [ "${1:-}" != "--inner" ]; then
    exec env -u SEEN_DATA_PATH -u SEEN_STDLIB_PATH -u SEEN_RUNTIME_PATH \
        ...
        env QWN_041D_HARD_SCOPE=1 SEEN_TOOLCHAIN_ROOT="$TOOLCHAIN_ROOT" \
        "$0" --inner
fi

[ "${QWN_041D_HARD_SCOPE:-0}" = 1 ] || exit 126
...
python3 -m unittest tests/test_cuda_gdn_gated_output.py
"$SEEN_BIN" check "$ROOT_DIR/tests/qwn_041d_gdn_output_test.seen" --frozen
"$SEEN_BIN" compile "$ROOT_DIR/tests/qwn_041d_gdn_output_test.seen" \
    "$ARTIFACT_ROOT/qwn_041d_seen_surface_test" \
    --release --lto=thin --target-cpu=x86-64 --no-cache \
    --jobs 1 --opt-jobs 1 --no-fork --frozen
"$ARTIFACT_ROOT/qwn_041d_seen_surface_test"
QWN_041C_HARD_SCOPE=1 "$ROOT_DIR/scripts/cuda/run_qwn_041c.sh" --inner
...
for tool in memcheck initcheck racecheck synccheck; do
    ...
    /opt/cuda/bin/compute-sanitizer "${args[@]}" "$BUILD_ROOT/qwn_041d_cuda_test"
done
sha256sum "$BUILD_ROOT/libseen_qwen_cuda.so" "$BUILD_ROOT/qwn_041d_cuda_test" \
    "$BUILD_ROOT/seen_cuda_build/libseen_cuda.so.1.0.0"
echo "PASS: QWN-041D v0.20.4 complete GDN CUDA differential gates"
