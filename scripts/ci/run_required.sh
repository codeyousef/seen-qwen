...
TOOLCHAIN_ROOT="$SHARED_ROOT/.seen/toolchains/v0.20.8/extracted/seen-0.20.8-linux-x64"
...
[ -d "$TOOLCHAIN_ROOT" ] && [ ! -L "$TOOLCHAIN_ROOT" ] ||
    fail "shared v0.20.8 toolchain root is missing or unsafe"
...
docker run --rm --platform linux/amd64 \
    ...
    --mount "type=bind,src=$TOOLCHAIN_ROOT,dst=/workspace/.seen/toolchains/seen-0.20.8-linux-x64,readonly" \
